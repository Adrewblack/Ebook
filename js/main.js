...
window.addEventListener("DOMContentLoaded", () => {
  ...
  searchInput.addEventListener("keydown", function (event) {
    if (event.key === "Enter") {
      ...
      fetch(
        `https://openlibrary.org/people/mekBot/books/currently-reading.json?q=${searchTerm}`
      )
        .then((res) => res.json())
        .then((data) => {
          if (data.reading_log_entries && data.reading_log_entries.length > 0) {
            for (let i = 0; i < data.reading_log_entries.length; i++) {
              ...
              bookAuthor.textContent =
                data.reading_log_entries[i].work.author_names;
  ...
  //             cardBody.classList.add("card-body");
  //             let cardImg = document.createElement("img");
  //             cardImg.src = `https://covers.openlibrary.org/b/olid/${data.reading_log_entries[i].cover_edition_key}.jpg`;
  //             cardImg.style = "width: 200px; height:200px";
  //             cardBody.append(cardImg);
  //             let cardFooter = document.createElement("div");
  //             cardFooter.classList.add("card-footer");
  ...
  //             let bookName = document.createElement("span");
  //             bookName.classList.add("book-name");
  //             bookName.textContent = data.reading_log_entries[i].title;
  //             let bookAuthor = document.createElement("p");
  //             bookAuthor.classList.add("author");
  //             bookAuthor.textContent = data.reading_log_entries[i].author_names;
  //             let bookDate = document.createElement("p");
  //             bookDate.classList.add("book-date");
  //             bookDate.textContent =
  //               data.reading_log_entries[i].first_publish_year;
  //             productDesc.append(bookName, bookAuthor, bookDate);
  //             let buttons = document.createElement("div");
  //             buttons.classList.add("buttons");
  //             let bookmarkBtn = document.createElement("button");
